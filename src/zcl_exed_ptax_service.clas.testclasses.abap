CLASS quote_source_double DEFINITION FINAL.
  PUBLIC SECTION.
    INTERFACES zif_exed_ptax_source.
    DATA absent TYPE zif_exed_ptax_types=>bacen_currency_code.
    DATA all_absent TYPE abap_bool.
    DATA fail TYPE abap_bool.
    DATA calls TYPE i.
    DATA last_date TYPE d.
ENDCLASS.
CLASS quote_source_double IMPLEMENTATION.
  METHOD zif_exed_ptax_source~get_quote.
    ...
    ENDIF.
    quote = VALUE #( currency = currency quotation_date = requested_date
      found = xsdbool( all_absent = abap_false AND absent <> currency )
      buy_rate = '4.99560' bulletin_timestamp = '2024-03-28 14:40:02.052' ).
  ENDMETHOD.
ENDCLASS.
CLASS calendar_double DEFINITION FINAL.
  PUBLIC SECTION.
    INTERFACES zif_exed_ptax_calendar.
    DATA calls TYPE i.
ENDCLASS.
CLASS calendar_double IMPLEMENTATION.
  METHOD zif_exed_ptax_calendar~previous_workday.
    calls += 1.
    workday = '20240328'.
  ENDMETHOD.
ENDCLASS.
CLASS rate_store_double DEFINITION FINAL.
  PUBLIC SECTION.
    INTERFACES zif_exed_ptax_rate_store.
    DATA inspected TYPE zif_exed_ptax_types=>currency_pairs.
    DATA applied TYPE zif_exed_ptax_types=>rate_items.
    DATA inspect_calls TYPE i.
    DATA apply_calls TYPE i.
    DATA fail_currency TYPE zif_exed_ptax_types=>bacen_currency_code.
ENDCLASS.
CLASS rate_store_double IMPLEMENTATION.
  METHOD zif_exed_ptax_rate_store~inspect.
    inspect_calls += 1.
    inspected = currency_pairs.
    LOOP AT currency_pairs INTO DATA(pair).
      APPEND VALUE #( source_currency = pair-source_currency
        ...
        quotation = pair-quotation effective_date = requested_date
        action = zif_exed_ptax_types=>action_create ) TO result_items.
    ENDLOOP.
  ENDMETHOD.
  METHOD zif_exed_ptax_rate_store~apply.
    ...
    applied = items.
    result_items = items.
    LOOP AT result_items ASSIGNING FIELD-SYMBOL(<item>) WHERE bacen_currency = fail_currency.
...
CLASS service_tests DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    DATA source TYPE REF TO quote_source_double.
    DATA calendar TYPE REF TO calendar_double.
    DATA store TYPE REF TO rate_store_double.
...
ENDCLASS.
CLASS service_tests IMPLEMENTATION.
  METHOD setup.
    source = NEW #( ).
    calendar = NEW #( ).
    store = NEW #( ).
    service = NEW #( source = source calendar = calendar store = store ).
  ENDMETHOD.
  METHOD missing_usd.
    source->absent = 'USD'.
    DATA(result) = service->run( reference_date = '20240401' simulate = abap_false ).
  ...
  METHOD simulation_never_writes.
    DATA(result) = service->run( reference_date = '20240401' ).
  ...
  METHOD explicit_date.
    DATA(result) = service->run( reference_date = '20240401' quotation_date = '20240327' ).
  ...
  METHOD http_failure_never_writes.
    ...
    TRY.
        service->run( reference_date = '20240401' simulate = abap_false ).
